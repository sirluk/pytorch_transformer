...
def func(global_rank, local_rank, world_size, train_cfg, model_cfg):
    ...
    # model = TransformerContext(model_cfg)
    ...
    for step in train_iterator:
        ...
        if step % train_cfg.eval_interval == 0:
            ...
            with torch.no_grad():
                for eval_step, (x, y) in enumerate(eval_iterator):
                    ...
                    losses[eval_step] = out_eval['loss']
                    context_losses[eval_step] = out_eval['context_loss']
        ...
        if world_size > 1:
            dist.all_reduce(out['loss'].detach(), op=dist.ReduceOp.SUM)
            dist.all_reduce(out['context_loss'].detach(), op=dist.ReduceOp.SUM)
        if global_rank == 0:
            log_dict = {
                "train_loss": out['loss'].item() / world_size,
                'train_context_loss': out['context_loss'].item() / world_size,
